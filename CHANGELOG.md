# Changelog

## Unreleased

- Improve installation docs and add `install.sh`.
- Group selectable roles by category.
- Add public figure optional roles with source attribution.
- Rename built-in roles for clearer selection.
- Add round summaries with consensus, disagreement, follow-up questions, and next step.
- Replace blockquote role cards with stable plain-text role-card blocks.
- Resolve link context with matching local tools before saying a link is inaccessible.
- Prohibit ambiguous first-person summary sections such as `我的阶段判断`.
- Switch default discussion output to compact briefing cards with expandable detail prompts.
- Make round summaries preserve complete consensus and structure disagreements as role-vs-role bullets.

## 0.1.0

- Initial Challenge Master skill release.
- Add dynamic role recommendation.
- Add role library and output templates.
- Support brainstorm notes and requirement cards.
