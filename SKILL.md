...
你要不要加人或换人？例如可以加入：<2-4 个备选角色>。
```

Do not start role debate until the user has had a chance to confirm, add, or swap roles. If the user says “你定”, “就这样”, “继续”, or gives equivalent approval, proceed with the recommended panel.
...
- `方案军师`: turns ideas into options, trade-offs, MVPs, and validation paths.
- `高级产品总监`: judges strategy fit, priority, resources, and product direction.
- `用户运营经理`: evaluates segmentation, lifecycle, touchpoints, benefits, and retention.
- `增长运营经理`: evaluates conversion, growth experiments, metrics, and ROI.
- `老板视角`: evaluates business outcome, revenue, cost, risk, and executive story.
...
- `客服体验专家`: checks support quality, escalation, SLA, and service recovery.
- `运营提效专家`: checks workflow savings, adoption friction, SOP fit, and owners.
...
- 方案军师 challenges critics to propose a smaller validation path instead of only saying no.
- 数据分析师 challenges any role that makes a claim without measurable evidence.
- 老板视角 challenges business impact, cost, and risk assumptions.
